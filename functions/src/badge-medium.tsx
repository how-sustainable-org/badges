import { dmSans, dmSerifDisplay } from "./load-fonts";
import colors from "tailwindcss/colors";

export type BadgeMediumProps = {
  scores: {
    how_sustainable_rank_avg: number
    how_sustainable_label: string
    company_name: string
  }
};

export function BadgeMedium(props: BadgeMediumProps) {
  const { scores } = props;
  const ranking = scores.how_sustainable_rank_avg
  const label = scores.how_sustainable_label
  const company_name = scores.company_name
  const isPositive = ranking >= 50;
  ...
  const symbol = (
    ...
    >
      ...
      <defs>
        ...
        >
          <stop
            stop-color={isPositive ? colors.emerald[700] : colors.rose[800]}
          />
          <stop
            offset="1"
            stop-color={isPositive ? colors.emerald[400] : colors.rose[500]}
          />
        </linearGradient>
      </defs>
    </svg>
    ...
    >
      ...
      <div
        style={{
          ...
          fontSize: 40,
          letterSpacing: -0.5,
          color: isPositive ? colors.emerald[400] : colors.rose[500],
          margin: "24px 0px",
        }}
      >
        {isPositive ? 100 - ranking : label}
      </div>
    </div>
  );
}
